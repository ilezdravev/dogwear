  ...
  plus[i].onclick = function() {
    ...
    if (classes.contains("show")) {
      ...
      if (previousTextOverImage != null)
		...
		this.classList.remove("open");
      previousTextOverImage = this.nextElementSibling;
      classes.add("show");
...
//Accordion
  ...
  let accItems = document.querySelectorAll("#acc_item");
  let accImages = document.querySelectorAll("#img_item");
  ...
  function toggleAcc() {
    ...
    let plus = this.children[1].firstElementChild;
    // remove active class from all items exept the current item (this)
    accItems.forEach((item,i) => {
      if (item != this) {
        //change accordion to not autohide opened item
        item.classList.remove("accordion__item--active");
        // accImages[i].classList.remove("acc_image--active");
        item.children[1].firstElementChild.classList.remove("acc_plus--active");
      }
    }
    );
    ...
    if (this.classList != "accordion__item--active") {
      ...
      // img.classList.toggle("acc_image--active");
      this.children[1].firstElementChild.classList.toggle("acc_plus--active");
    }
  }